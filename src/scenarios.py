...
def default_scenario() -> List[LandingModuleConfig]:
    return [
        LandingModuleConfig("LSS-01", "Life Support System",    1, 78.0,  4_200.0, Criticality.VITAL,   0.5),
        LandingModuleConfig("PWR-01", "Power Generation",       2, 85.0,  6_800.0, Criticality.VITAL,   1.0),
        LandingModuleConfig("HAB-01", "Inflatable Habitat",     3, 62.0, 12_400.0, Criticality.HIGH,    2.0),
        LandingModuleConfig("MED-01", "Medical Support",        4, 71.0,  3_100.0, Criticality.HIGH,    2.5),
        LandingModuleConfig("SCI-01", "Science Laboratory",     5, 90.0,  5_600.0, Criticality.MEDIUM,  4.0),
        LandingModuleConfig("LOG-01", "Logistics and Supplies", 6, 55.0, 18_200.0, Criticality.MEDIUM,  5.5),  # SIMULATED — below FUEL_MIN_PCT to trigger alert
        LandingModuleConfig("MIN-01", "ISRU Mining",            7, 48.0,  9_900.0, Criticality.LOW,     7.0),  # SIMULATED — below FUEL_MIN_PCT to trigger alert
    ]


def random_scenario(
    n: int = 7,
    anomaly_pct: float = 0.25,
    seed: Optional[int] = None,
) -> List[LandingModuleConfig]:
    """Procedural generator — n modules with item-level anomaly_pct probability of low fuel."""
    ...
    for i in range(n):
        ...
        orbit_bump = round(rng.uniform(4.1, 8.0), 2)   if sensor_anomaly else round(rng.uniform(0.5, 1.5), 2)
